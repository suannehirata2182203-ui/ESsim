...
(function() {
    ...
    function setDefaults() {
        // Default: 500 Mbps SMART (package 1583) для tarif1
        const fiber500Mb = document.querySelector('[data-package="1583"][data-product-type="fiber"]');
        if (fiber500Mb) {
            selectFiber(fiber500Mb);
        }
        
        // Default: 25GB x1 (uuid 1286) from URL parameter
        const mobile25gb = document.querySelector('[data-uuid="1286"]');
        if (mobile25gb) {
            const valueSpan = mobile25gb.querySelector('.value');
            if (valueSpan) {
    ...
    function updateMobileState() {
        ...
        document.querySelectorAll('.product_row[data-uuid]').forEach(row => {
            ...
            if (count > 0) {
                ...
                if (priceDiv) {
                    const priceSpan = priceDiv.querySelector('span:not(small)');
                    if (priceSpan) {
